...
This is not a hypothetical crash window. Production LSM-tree engines such as [LevelDB](https://github.com/google/leveldb/blob/main/doc/impl.md) and [RocksDB](https://github.com/facebook/rocksdb/wiki/MANIFEST) also separate the logical installation of a new database version from the later deletion of obsolete files. LevelDB persists version changes through its `MANIFEST`/`VersionSet` machinery — `VersionSet::LogAndApply()` durably writes and syncs a version edit before `RemoveObsoleteFiles()` is ever called — while RocksDB uses `MANIFEST` version edits as the durable source of truth for live and obsolete SST state.

`compaction.pending` is a deliberately smaller, from-scratch mechanism for the same general class of crash-consistency problem. It does not attempt to reproduce a production database's full manifest/version-set system; instead, it protects StoneKV's full-compaction transaction around the critical install boundary. Tests cover rollback when interruption occurs before the compacted segment is installed, and recovery when the replacement exists but obsolete input segments have not yet been fully cleaned up, including prevention of deleted-key resurrection (see `store::tests::interrupted_compaction_before_install_rolls_back` and `store::tests::interrupted_compaction_does_not_resurrect_deleted_key`).
